...
! ::: -----------------------------------------------------------
  ...
  subroutine amrex_probinit (init,name,namlen,problo,probhi) bind(c)
      ...
      use mod_Fvar_def, only : ac_hist_file, cfix, changemax_control, &
                               ...
                               pseudo_gravity
      use probdata_module, only : standoff, pertmag, T_in, rho_bc, Y_bc
      use probdata_module, only : flame_dir, phi_lean, phi_rich, midtanh, widthtanh
      ...
      namelist /fortin/ V_in, T_in, standoff, pertmag, &
                        phi_lean, phi_rich, midtanh, widthtanh
...
!     Note: for setup with no coflow, set Ro=Rf+wallth
      ...
      phi_lean = 0.0d0
      phi_rich = 5.0d0
      midtanh = 0.5*(domnhi(1)+domnlo(1))    ! Default is middomain in x
      widthtanh = 0.1*(domnhi(1)-domnlo(1))  ! Default is 1/10th of domain width in x 
...
!------------------------------------
  ...
  subroutine setupbc()bind(C, name="setupbc")
    ...
    use probdata_module, only : bcinit, phi_rich, phi_lean, T_in
    use user_defined_fcts_2d_module, only: set_Y_from_Phi
  ...
  ! Get the lean and rich mixture comp
    call set_Y_from_Phi(phi_rich,Y_bc(0:nspec-1,1))     
    call set_Y_from_Phi(phi_lean,Y_bc(0:nspec-1,2))     
...
! ::: -----------------------------------------------------------
  ...
  subroutine init_data(level,time,lo,hi,nscal, &
      ...
      use mod_Fvar_def, only : bathID, domnhi, domnlo, maxspec, maxspnml , V_in
      use probdata_module, only : Y_bc, rho_bc, T_bc, midtanh, widthtanh
...
!      write(6,*)" made it to initdata"
      ...
      do j = lo(2), hi(2)
        ...
        do i = lo(1), hi(1)
          ...
          ! Start with tanh  between phi_rich and phi_lean
          tanhval = 0.5d0*(1.0d0+TANH((x-midtanh)/widthtanh))
          scal(i,j,Temp) = T_bc(1)
          DO n = 1, nspec
            scal(i,j,FirstSpec+n-1) = Y_bc(n-1,2) + tanhval * &
                                      (Y_bc(n-1,1) - Y_bc(n-1,2))
          END DO

          rad = SQRT((x-0.005)**2.0+(y-0.01)**2.0)
          if ( rad < 0.002 ) scal(i,j,Temp) = 1700.0d0

          y1 = (y - 0.5d0*delta(2) + pert)*100.d0
          y2 = (y - 0.5d0*delta(2) + pert)*100.d0
...
!            call pmf(y1,y2,pmf_vals,nPMF)               
          ...
          vel(i,j,1) = 0.d0
          vel(i,j,2) = V_in + tanhval * (V_in*rho_bc(1,2)/rho_bc(1,1) - V_in)
